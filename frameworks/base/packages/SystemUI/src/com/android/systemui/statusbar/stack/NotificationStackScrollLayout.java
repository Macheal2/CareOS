...
public class NotificationStackScrollLayout extends ViewGroup
    ...
    private int mMaxLayoutHeight;//hmq notification view Max Height
    ...
    private boolean mTouchIsClick;//hmq 是否点击效果，touch down 状态为true，如果拖动状态就是false
    private float mInitialTouchX;
    private float mInitialTouchY;

    private int mSidePaddings;//hmq notification 通知的边界填充 8dp
    private Paint mDebugPaint;
    private int mContentHeight;//hmq 内容高度
    private int mCollapsedSize;//hmq 状态栏中的小通知的高度64dp
    private int mBottomStackSlowDownHeight;
    private int mBottomStackPeekSize;//hmq 栈底可见间距 12dp
    ...
    private boolean mExpandingNotification;//hmq 是否展开通知栏
    private boolean mExpandedInThisMotion;
    private boolean mScrollingEnabled;//hmq 是否可以滑动
    private DismissView mDismissView;
    private EmptyShadeView mEmptyShadeView;
    private boolean mDismissAllInProgress;
    private int mNotificationHeightOffset;//hmq
     ...
     * 在扩展卡上滚动时，要切换到快速设置，以切换到快速设置的最小量。36dp
     */
    ...
    private ViewGroup mScrollView;//hmq 包含QS
    private boolean mInterceptDelegateEnabled;
    private boolean mDelegateToScrollView;
    private boolean mDisallowScrollingInThisMotion;
    private long mGoToFullShadeDelay;
    protected boolean notNeedCheckLR;//add by hmq 20160504状态栏分2页显示
    ...
    public NotificationStackScrollLayout(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        int minHeight = getResources().getDimensionPixelSize(R.dimen.notification_min_height);//hmq notification list item min height 64dip
        int maxHeight = getResources().getDimensionPixelSize(R.dimen.notification_max_height);//hmq notification list item max height 256dip
        ...
        initView(context);
        setWillNotDraw(false);//add by hmq
        if (DEBUG) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // edit by hmq 添加通知栏填充背景颜色 
        //liukun
        if (/*getTranslationX() != getWidth() &&*/mPhoneStatusBar.getBarState() != StatusBarState.KEYGUARD && mInterceptDelegateEnabled) {
            mDebugPaint = new Paint();
            mDebugPaint.setStyle(Paint.Style.FILL);
            mDebugPaint.setColor(0xFFF2F0EE);
            int y = (int) getLayoutHeight();
            if(y <= 300){
                y = 50;
            }
            canvas.drawRect(0, 0, getWidth(), y, mDebugPaint);
        }
        // edit by hmq 
    }
    ...
    private void initView(Context context) {
        ...
        mSidePaddings = context.getResources()
                .getDimensionPixelSize(R.dimen.notification_side_padding);//8dp
        mCollapsedSize = context.getResources()
                .getDimensionPixelSize(R.dimen.notification_min_height);//64dp
        mBottomStackPeekSize = context.getResources()
                .getDimensionPixelSize(R.dimen.bottom_stack_peek_amount);//12dp
        ...
        mMinTopOverScrollToEscape = getResources().getDimensionPixelSize(
                R.dimen.min_top_overscroll_to_qs);//36dp
        mNotificationTopPadding = getResources().getDimensionPixelSize(
                R.dimen.notifications_top_padding);//0dp
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int childMeasureSpec = MeasureSpec.makeMeasureSpec(size /* - 2 * mSidePaddings*/, mode);//edit by hmq 20160504状态栏分2页显示 让notification没有边界
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        setMaxLayoutHeight(getHeight() - mNotificationHeightOffset);//edit by hmq 20160504状态栏分2页显示 mNotificationHeightOffset
        updateContentHeight();
        clampScrollPosition();
    ...
    /**
     * 
     * @param maxLayoutHeight 设置最大notification view Max Height
     */
    ...
    public void setStackHeight(float height) {
        ...
        if (newStackHeight - mTopPadding - mTopPaddingOverflow >= normalUnfoldPositionStart
            ...
            if (!trackingHeadsUp) {
/* Cappu:liukun on: Sat, 02 Sep 2017 20:12:03 +0800
 * TODO: 状态栏下拉 通知界面 上拉过程中会剩下一块空白问题
 */
                //translationY += (1 - partiallyThere) * (mBottomStackPeekSize + mCollapseSecondCardPadding);
            	translationY += (1 - partiallyThere) * (mCollapseSecondCardPadding);
// End of Cappu:liukun
            } else {
    ...
    //edit by hmq 20160504状态栏分2页显示 private--public
...
//        handleEmptySpaceClick(ev);//hide by hmq 20160504状态栏分2页显示
        ...
        return horizontalSwipeWantsIt || scrollerWantsIt || expandWantsIt || super.onTouchEvent(ev) || emptyChild;
    ...
    /**
     * @return 是否把对顶OverScroll边缘导致扩大OverScroll视图（即QS）
    ...
    private boolean shouldOverScrollFling(int initialVelocity) {
        ...
        return mScrolledToTopOnFirstDown
                && !mExpandedInThisMotion
                && topOverScroll > mMinTopOverScrollToEscape //顶部滚动是否大于最小下拉的量
                && initialVelocity > 0;
    }
...
//                }
    ...
    /* begin: add by hmq 20160504状态栏分2页显示 */
    public void setNotificationHeightOffset(int offsetHeight){
        mNotificationHeightOffset = offsetHeight - 3;
    }
    /* end: add by hmq 20160504状态栏分2页显示 */
}
