        ...
        if (gsmRscpQdbm < 0) {
            ...
            if (asu <= 5 || asu == 99) {
                ...
                level = SignalStrength.SIGNAL_STRENGTH_GREAT;
                 ...
                 //modify by zhouhua: improve signal level
                level = SignalStrength.SIGNAL_STRENGTH_GREAT;
    ...
    public int mapLteSignalLevel(int mLteRsrp, int mLteRssnr, int mLteSignalStrength) {
        ...
        if (CdmaFeatureOptionUtils.isCT6MSupport()) {
            if (isCUCardInserted()) {
                ...
                Log.d(TAG, "op09C_CU_mapLteSignalLevel=" + rsrpIconLevel);
    ...
    */
    ...
    private boolean isCUCardInserted() {
        ...
        for (phoneId = 0; phoneId < TelephonyManager.getDefault().getPhoneCount(); phoneId++) {
            //get from framework, later than native, maybe empty
            simOperator = TelephonyManager.getTelephonyProperty(
                phoneId, TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC, "");
            Log.d(TAG, "isCUCardInserted, Phone[" + phoneId + "] mccmnc:" + simOperator +
                    ",from " + TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC);
